...
MP3DecInfo *AllocateBuffers(void)
{
    ...
    mp3DecInfo = (MP3DecInfo *)audio_malloc(sizeof(MP3DecInfo));
    if (!mp3DecInfo)
        return 0;
    ClearBuffer(mp3DecInfo, sizeof(MP3DecInfo));

    fh =  (FrameHeader *)     audio_malloc(sizeof(FrameHeader));
    si =  (SideInfo *)        audio_malloc(sizeof(SideInfo));
    sfi = (ScaleFactorInfo *) audio_malloc(sizeof(ScaleFactorInfo));
    hi =  (HuffmanInfo *)     audio_malloc(sizeof(HuffmanInfo));
    di =  (DequantInfo *)     audio_malloc(sizeof(DequantInfo));
    mi =  (IMDCTInfo *)       audio_malloc(sizeof(IMDCTInfo));
    sbi = (SubbandInfo *)     audio_malloc(sizeof(SubbandInfo));
...
#define SAFE_FREE(x)    {if (x) audio_free(x);    (x) = 0;}   /* helper macro */
