...
int MP3GetNextFrameInfo(HMP3Decoder hMP3Decoder, MP3FrameInfo *mp3FrameInfo, unsigned char *buf)
{
    ...
    if (UnpackFrameHeader(mp3DecInfo, buf) == -1 || mp3DecInfo->layer != 3) {
        //printf("Invalid header 1st: [0x%2X, 0x%2X, 0x%2X, 0x%2X]\n", buf[0], buf[1], buf[2], buf[3]);
        return ERR_MP3_INVALID_FRAMEHEADER;
    }

    if(mp3DecInfo->frameSyncFind == false) {
        /* Calculate frame size and check whether it is fake frame header */
        
        FrameHeader *fh = (FrameHeader *)(mp3DecInfo->FrameHeaderPS);

        int pad_slot        = (fh->paddingBit & 0x01) ? 1 : 0;
        int samplerate      = mp3DecInfo->samprate;
        int bitrate         = mp3DecInfo->bitrate;
        int frame_size      = 0;
        
#define MP3_LAYER_I     1       
#define MP3_LAYER_II    2
#define MP3_LAYER_III   3        

        if (fh->layer == MP3_LAYER_I) {
            frame_size = ((12 * bitrate / samplerate) + pad_slot) * 4;
        } else {
            unsigned int slots_per_frame;

            slots_per_frame = (fh->layer == MP3_LAYER_III &&
                              (fh->ver == MPEG25 || fh->ver == MPEG2)) ? 72 : 144;

            frame_size = (slots_per_frame * bitrate / samplerate) + pad_slot;
        }

        if (UnpackFrameHeader(mp3DecInfo, buf+frame_size) == -1 || mp3DecInfo->layer != 3) {
            //char *data = buf+frame_size;
            //printf("Invalid header 2nd(frame_size=0x%X):[0x%2X,0x%2X,0x%2X,0x%2X]\n", frame_size, data[0], data[1], data[2], data[3]);
            return ERR_MP3_INVALID_FRAMEHEADER;
        } else {
            mp3DecInfo->frameSyncFind = true;
        }
    }
