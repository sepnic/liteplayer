...
typedef struct mp3_player* mp3_player_handle_t;
typedef esp_err_t (*player_event)(mp3_player_handle_t ap, player_event_t event);

typedef struct {
    player_event event_handler;
} mp3_player_config_t;

#define DEFAULT_MP3_PLAYER_CONFIG() {\
    .event_handler   = NULL, \
}
...
{
    mp3_player_handle_t ap = (mp3_player_handle_t) ctx;
    ...
    if (msg->source_type == AUDIO_ELEMENT_TYPE_ELEMENT) {
        ...
        if (msg->cmd == AEL_MSG_CMD_REPORT_STATUS) {
            ...
            case AEL_STATUS_STATE_RUNNING:
                if (msg->source == (void *) ap->mp3_decoder) {
            ...
            if (msg->source == (void *) ap->mp3_decoder) {
                ...
                audio_element_getinfo(ap->mp3_decoder, &info);
...
static int mp3_fetch_data(char *buf, int wanted_size, long offset, void *priv)
{
    const char *url = (const char *)priv;
    FILE *file = NULL;
    int ret = -1;
    ...
    if (file != NULL) {
        ...
        ret = fread((void *)buf, 1, wanted_size, file);
        fclose(file);
    }
    return ret;
}

static void mp3_player_deinit(mp3_player_handle_t ap)
{
    if (ap == NULL)
        return;
...
static mp3_player_handle_t mp3_player_init(mp3_player_config_t *config)
{
    mp3_player_handle_t ap = audio_calloc(1, sizeof(struct mp3_player));
    ...
    AUDIO_MEM_CHECK(TAG, ap->mp3_decoder, goto audio_init_failed);

    ESP_LOGI(TAG, "[2.3] Register all elements to audio pipeline");
    audio_pipeline_register(ap->pipeline, ap->fatfs_stream_reader, "fatfs_r");
    audio_pipeline_register(ap->pipeline, ap->mp3_decoder,         "mp3_dec");
    audio_pipeline_register(ap->pipeline, ap->fatfs_stream_writer, "fatfs_w");

    ESP_LOGI(TAG, "[2.4] Link elements together fatfs_stream_r->mp3_decoder->fatfs_stream_w");
    audio_pipeline_link(ap->pipeline, (const char *[]) {"fatfs_r", "mp3_dec", "fatfs_w"}, 3);
    ...
    audio_element_set_event_callback(ap->mp3_decoder, audio_element_state_cb, ap);
...
audio_init_failed:
    mp3_player_deinit(ap);
    return NULL;
}

static esp_err_t mp3_player_start(mp3_player_handle_t ap, const char *in_mp3, const char *out_wav)
{
    if (in_mp3 != NULL && out_wav != NULL) {
        mp3_info_t mp3_info;
        memset(&mp3_info, 0x0, sizeof(mp3_info));
        // find mp3 frame head to parse info
        int ret = mp3_extractor(mp3_fetch_data, (void *)in_mp3, &mp3_info);
        if (ret == 0) {
            audio_element_info_t write_info;
            audio_element_getinfo(ap->fatfs_stream_writer, &write_info);
            write_info.in_channels = mp3_info.channels;
            write_info.in_samplerate = mp3_info.sample_rate;
            write_info.bits = 16;
            audio_element_setinfo(ap->fatfs_stream_writer, &write_info);

            // skip id3v2 for decoder
            audio_element_info_t reader_info;
            audio_element_getinfo(ap->fatfs_stream_reader, &reader_info);
            reader_info.byte_pos = mp3_info.frame_start_offset;
        ...
        audio_element_set_uri(ap->fatfs_stream_reader, in_mp3);
...
/*static esp_err_t mp3_player_pause(mp3_player_handle_t ap)
{
    return audio_pipeline_pause(ap->pipeline);
}

static esp_err_t mp3_player_resume(mp3_player_handle_t ap)
{
    return audio_pipeline_resume(ap->pipeline, 0.0, 0);
}*/

static esp_err_t mp3_player_stop(mp3_player_handle_t ap)
{
...
static esp_err_t player_event_listerner(mp3_player_handle_t ap, player_event_t event)
{
    switch (event) {
...
{
    esp_err_t ret = ESP_OK;
    const char *in_mp3  = DEFAULT_IN_MP3;
    ...
    mp3_player_config_t config = DEFAULT_MP3_PLAYER_CONFIG();
    config.event_handler = player_event_listerner;
    mp3_player_handle_t ap = mp3_player_init(&config);
    if (ap == NULL) {
    ...
    ret = mp3_player_start(ap, in_mp3, out_wav);
    if (ret != ESP_OK) {
